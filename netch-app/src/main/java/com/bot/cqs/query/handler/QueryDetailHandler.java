 ...
 *          </ul>
 */
@Controller("querydetailhandler")
public class QueryDetailHandler extends NetchMFormHandler {

    private static final String FUNCTION_ID = "L020";
    ...
    @Resource private EtchAuditLogService etchAuditLogService;
    ...
    public BeanGridResult query(SearchSetting search, Request request) {
        ...
        sessionLogService.createAndWriteSessionLog(request, SessionLog.PRIMARY_STATUS_QUERY_INQUIRYLOG, SessionLog.SECONDARY_STATUS_NO_MORE_DESCRIPTION, request.get("action", ""), null);
        // 寫 EtchAuditLog
    ...
    public Result checkIfDataExist(Request request) {
        ...
        if (startDate != null && endDate != null) {
            ...
            if (days > cyale) {
                throw new NetchLogMessageException(CapAppContext.getMessage("js.queryDetail.msg.03", new Object[] { cyale }), this.getClass());// 已超過最大週期天數： [{0}] 天
            }
        }
        List<InquiryLog> list = queryForPage(search, request).getContent();
        if (CollectionUtils.isEmpty(list)) {
            sessionLogService.createAndWriteSessionLog(request, SessionLog.PRIMARY_STATUS_QUERY_INQUIRYLOG, SessionLog.SECONDARY_STATUS_DATA_NOT_FOUND, request.get("action", ""), null);
    ...
    public Result downloadCsv(Request request) {
        ...
        try {
            ...
            if (csvString != null) {
                ...
                return new ByteArrayDownloadResult(request, input, "text/plain", "queryDetail.csv");
            }
    ...
    public Result downloadPdf(Request request) throws FileNotFoundException {
        ...
        return new ByteArrayDownloadResult(request, outputstream.toByteArray(), ContextTypeEnum.pdf.toString(), "queryDetail.pdf");
    }
}
