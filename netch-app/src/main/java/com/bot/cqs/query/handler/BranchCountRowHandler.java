 ...
 *          </ul>
 */
@Controller("branchcountrowhandler")
public class BranchCountRowHandler extends NetchMFormHandler {

    private static final String FUNCTION_ID = "L045";
    ...
    public MapGridResult query(SearchSetting search, Request request) {
        ...
        sessionLogService.createAndWriteSessionLog(request, SessionLog.PRIMARY_STATUS_COUNT_ROW_BY_BRANCH, SessionLog.SECONDARY_STATUS_NO_MORE_DESCRIPTION, request.get("action", ""), null);
    ...
    public Result checkIfDataExist(Request request) {
        ...
        if (CollectionUtils.isEmpty(list)) {
            sessionLogService.createAndWriteSessionLog(request, SessionLog.PRIMARY_STATUS_COUNT_ROW_BY_BRANCH, SessionLog.SECONDARY_STATUS_DATA_NOT_FOUND, request.get("action", ""), null);
    ...
    public Result downloadCsv(Request request) {
        ...
        try {
            ...
            if (csvString != null) {
                sessionLogService.createAndWriteSessionLog(request, SessionLog.PRIMARY_STATUS_COUNT_ROW_BY_BRANCH, SessionLog.SECONDARY_STATUS_NO_MORE_DESCRIPTION, request.get("action", ""), null);
                return new ByteArrayDownloadResult(request, input, "text/plain", "branchCountRow.csv");
            }
    ...
    public Result downloadPdf(Request request) throws FileNotFoundException {
        ...
        sessionLogService.createAndWriteSessionLog(request, SessionLog.PRIMARY_STATUS_COUNT_ROW_BY_BRANCH, SessionLog.SECONDARY_STATUS_NO_MORE_DESCRIPTION, request.get("action", ""), null);
        return new ByteArrayDownloadResult(request, outputstream.toByteArray(), ContextTypeEnum.pdf.toString(), "branchCountRow.pdf");
    }
}
