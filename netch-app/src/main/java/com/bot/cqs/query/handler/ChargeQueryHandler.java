 ...
 *          </ul>
 */
@Controller("chargequeryhandler")
public class ChargeQueryHandler extends NetchMFormHandler {

    private static final String FUNCTION_ID = "L010";
    ...
    public MapGridResult query(SearchSetting search, Request request) {
        ...
        sessionLogService.createAndWriteSessionLog(request, SessionLog.PRIMARY_STATUS_CHARGE_INQUIRYLOG, SessionLog.SECONDARY_STATUS_NO_MORE_DESCRIPTION, request.get("action", ""), null);
        MapGridResult mapGridResult = new MapGridResult(resultList, totalCnt);
    ...
    public Result checkIfDataExist(Request request) {
        ...
        if (CollectionUtils.isEmpty(list)) {
            sessionLogService.createAndWriteSessionLog(request, SessionLog.PRIMARY_STATUS_CHARGE_INQUIRYLOG, SessionLog.SECONDARY_STATUS_DATA_NOT_FOUND, request.get("action", ""), null);
    ...
    public Result downloadCsv(Request request) {
        ...
        try {
            ...
            if (csvString != null) {
                sessionLogService.createAndWriteSessionLog(request, SessionLog.PRIMARY_STATUS_CHARGE_INQUIRYLOG, SessionLog.SECONDARY_STATUS_NO_MORE_DESCRIPTION, request.get("action", ""), null);
                return new ByteArrayDownloadResult(request, input, "text/plain", "chargeQuery.csv");
            }
    ...
    public Result downloadPdf(Request request) throws FileNotFoundException {
        ...
        sessionLogService.createAndWriteSessionLog(request, SessionLog.PRIMARY_STATUS_CHARGE_INQUIRYLOG, SessionLog.SECONDARY_STATUS_NO_MORE_DESCRIPTION, request.get("action", ""), null);
        return new ByteArrayDownloadResult(request, outputstream.toByteArray(), ContextTypeEnum.pdf.toString(), "chargeQuery.pdf");
    }
