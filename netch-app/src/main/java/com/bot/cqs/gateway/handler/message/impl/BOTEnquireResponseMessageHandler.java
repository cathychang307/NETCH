 ...
 */
public class BOTEnquireResponseMessageHandler implements ResponseMessageHandler<InquiryLog, String> {
     ...
     *             若處理過程中有異常發生時拋出
     */
    public InquiryLog parse(String result, String msgId, String id, String account) throws GatewayException {
        ...
        try {
            msgr = InquiryLog.class.cast(core.transform(response, id, InquiryLog.class));

            if (!msgId.equals(msgr.getMsgId()))
                throw new GatewayException("MG010", new String[] { msgId, msgr.getMsgId() });
            if (!"D0000".equals(msgr.getInquiryErrorCode()))
                throw new GatewayException(msgr.getInquiryErrorCode());
