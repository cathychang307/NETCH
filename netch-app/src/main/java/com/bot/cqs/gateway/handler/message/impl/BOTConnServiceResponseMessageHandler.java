 ...
 */
public class BOTConnServiceResponseMessageHandler implements ResponseMessageHandler<ServiceSessionMsg, String> {
     ...
     *             若處理過程中有異常發生時拋出
     */
    public ServiceSessionMsg parse(String result, String msgId, String id, String account) throws GatewayException {

        byte[] response = null;
        ServiceSessionMsg msgr;
        ...
        try {
            msgr = ServiceSessionMsg.class.cast(core.transform(response, id, ServiceSessionMsg.class));
            if (!id.equals(msgr.getTrxCode()))
                throw new GatewayException("MG992");
