...
/**
 * 此類別主要用於將使用者或前端系統輸入的登入或登出請求參數以及部分分行資訊，透過事先定義的訊息格式檔：messages.xml 將請求參數組裝成串列字串，透過IBM MQ上傳至票交所進行票信資料查詢。
 *
 * @author Jeff Tseng
 * @see ConnectMsg
 ...
 */
public class BOTLoginOrLogoffRequestMessageHandler implements RequestMessageHandler<String, ConnectMsg> {
     ...
     *             若處理過程中有異常發生時拋出
     */
    public String format(ConnectMsg queryCommand, boolean initQueryFlag) throws GatewayException {
        try {
