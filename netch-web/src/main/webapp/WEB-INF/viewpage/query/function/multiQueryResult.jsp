...
<title><spring:message code="multiQueryResult.title">
        <!--查詢結果-->
...
<body>
    <div class="tr01 pr01">
        ...
        <form method="POST" id="actionForm" name="actionForm" class="form-horizontal" role="form" onsubmit="return false;" autocomplete="off">
            ...
            <div class="row hidden-print">
                <div class="form-group">
                    <div class="col-sm-10">
                        <spring:message code="multiQuery.result.thisInquiry">
                            <!-- 本次共查詢 -->
                        </spring:message>
                        ${fn:length(inquiryThreads) }
                        <spring:message code="multiQuery.result.rows">
                            <!-- 筆 -->
                        </spring:message>
                    </div>
                </div>
            ...
            <div class="btns hidden-print">
                <button type="button" id="print" name="print" class="btn btn-default">
                    <spring:message code="multiQuery.btn.print">
                        <!-- 列 印 -->
                    </spring:message>
                </button>
                <button type="button" id="previous" name="previous" class="btn btn-default"
                    onclick="$.ajax({url: url('multiqueryhandler/multiInquiry'), data: {'inputTransactionId': '${inputTransactionId }'}}).done(function(){router.to('function/multiQueryDetail');});">
                    <spring:message code="multiQuery.btn.previous">
                        <!-- 回到上頁 -->
                    </spring:message>
                </button>
        ...
        <c:forEach items="${inquiryThreads }" var="inquiry" varStatus="inquiryStatus">
            ...
            <ol>
                <li><span style="color: blue"><spring:message code="multiQuery.inquiry.which">
                            <!-- 第 -->
     ...
     --%>
                <div class="row row-flex">
                    <div class="col-sm-4 col-xs-12 td_style_1">
                        <span style="font-weight: bold"><spring:message code="multiQueryResult.state">
                                <!-- 查詢狀態 -->
                            </spring:message></span>
                    </div>
                    ...
                    <c:forEach items="${queryDef.queryInputFieldDefinition }" var="fieldDef" varStatus="fieldDefStatus">
                        ...
                        <div class="col-sm-8 col-xs-12 td_style_1">
                            <td>${paramMap[ currentFieldName ][0] }&nbsp;
                        </div>
                    </c:forEach>
                </div>
            </div>
            ...
            <c:if test="${ inquiry.status == 20 }">
                <c:set var="inquiryResponseHtml" value="${inquiry.inquiryLog.inquiryResponse}" />
                <c:set var="inquiryResponseHtml2" value="${fn:replace(inquiryResponseHtml, '</body>', '')}" />
                <c:set var="inquiryResponseResult" value="${fn:replace(inquiryResponseHtml2, '</html>', '')}" />
