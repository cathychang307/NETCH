            ...
            $.extend(window, {
                ...
                errorCheck: (function() {
                    ...
                    _errorCheck.rule = {
                        ...
                        AJAX_HANDLER_EXCEPTION: function(xhr, action, json) {
                            ...
                            var msg = xhr.status + " : " + xhr.statusText + ", " + json.AJAX_HANDLER_EXCEPTION;
                            API.loadPage("error", function() {
                              $("#errorMsgPage").html(msg);
                              $("#breadcrumb").html("");
                            });
                            return false;
                        },
                ...
                CommonAPI: {
                    ...
                    showErrorMessage: function(title, message, action) {
                      if($("#errorMsgSection").length == 1){
                        $("#errorMsgSection").val(title ? title : '');
                      }else{
                        return API.showPopMessage(title, message, action, "errorDialogCss");
                      }
                    },
                     ...
                     */
                    loadCombos: function(updateKeys, comboaction, funcCall) {
                        if (updateKeys === "")
                            return {};
          ...
          require([ 'jquery-ui' ], function(jqueryui) {
            $.fn.extend({
                ...
                reset: function() {
                    ...
                    $(this).find("input:text,.field").each(function(){
                      $(this).data("realValue", "");
                    });
                ...
                serializeData: function(noHide) {
                    var result = {},
                        self = $(this);
                    $(["input", "select", "textarea", "span.field", "button.dropdown-toggle"]).each(function(i, v) {
                ...
                setBootstrapOptions : function(options) {
                  //i: index
                  //options[i]: value
                  var ulElement = $(this).next("ul");
                  var space = $(this).data("space");
                  if($(this).data("space")){
                    ulElement.append("<li><a data-key='" + space + "' data-value='" + space + "' href='#'>" + space + "</a></li>");
                  }
                  $(this).setBootstrapOptionsSelection(space);
                  for(var i in options){
                    ulElement.append("<li><a data-key='" + i + "' data-value='" + options[i] + "' href='#'>" + options[i] + "</a></li>");
                  }
                },
                setBootstrapOptionsSelection : function(selectedOption) {
                  $(this).next("ul").find("li a").each(function(){
                    if($(this).data("value") == selectedOption){
                        var key = $(this).data("key");
                        var value = $(this).data("value");
                        var combotype = $(this).data("combotype");

                        var showValue = $(this).html() + '<span class="caret"></span>';
                        var submitValue;
                        if(combotype == "1" || combotype == "3"){
                          submitValue = showValue;
                        }else{
                          submitValue = $(this).data("key");
                        }

                        //show value
                        $(this).closest("ul").prev().html(showValue);
                        //submit value
                        $(this).closest("ul").prev().data('realValue', submitValue);
                        $(this).closest("ul").prev().data('maskRule', submitValue);
                    }
                  });
                },
                /**
                ...
                setOptions: function(options, append) {
                    return !this[0] ? $(this) : this[0].nodeName.toLowerCase() != 'select' ? $(this) : (function(o, a) {
                        ...
                        var to = {},
                            ...
                            dataInclude = s.attr("data-include") || "",
                            ops = ""
                            /*
                        ...
                        if (dataInclude){
                          for (var k in o || {}) {
                            if (new RegExp(dataInclude).test(k)){
                              _o[k] = o[k];
                            }
                          }
                          o = _o;
      ...
      router : {
          set : function(settings) {
              if (settings) {
                  ...
                  window._router.on('route', function() {
                      router.params = {};
                      if (router._params) {
